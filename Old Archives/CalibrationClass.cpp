	...
	if ( !outputCreated ) {
		...
		if ( calibrationPoints.size() == 5 ) {
			...
			if ( !shared->calibrationComplete ) {
				// Save image
				std::string imageFilename	 = "/home/tom/Code/nuring/logging/" + shared->Logging.filenamePng;
			...
			shared->Logging.variable1 = std::to_string( shared->Calibration.calibratedOffetMM.x );
			shared->Logging.variable2 = std::to_string( shared->Calibration.calibratedOffetMM.y );
			shared->Logging.variable3 = std::to_string( shared->Calibration.calibratedOffetMM.x );
