...
private:
	...
	// float T				 = 0.6f;	// planning horizon duration [s]
	// float timeLastUpdate = 0.0f;

	// cv::Point2f thetaInitial;
	...
	// void  UpdateThetaPlan();
	// float Tau( float tElapsed );
	// float TauDot( float tElapsed );
...
// 	void  GenerateAmplifierPWM();
// float ConstrainValues( float x, float minVal, float maxVal );
// 	float CalculateDistance( cv::Point3f pt1, cv::Point3f pt2 );
...
// 	cv::Point3f positionPrevious = cv::Point3f( 0.0f, 0.0f, 0.0f );	   // Initial MJ position
...
// 	// Filter values
// 	cv::Point2f filteredThetaDotMeasured = cv::Point2f( 0.0f, 0.0f );	 // Filtered angular velocity
// 	const float filterAlpha				 = 0.2f;						 // Tune between 0.1 and 0.4, // Smoothing constant for exponential filter (0.0 = no update, 1.0 = instant update)
// };
