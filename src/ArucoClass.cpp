		...
		if ( !arucoDetectedIDs.empty() ) {
			...
			for ( int i = 0; i < arucoDetectedIDs.size(); i++ ) {
				...
				if ( ( arucoDetectedIDs[i] > 0 && arucoDetectedIDs[i] <= 5 ) || ( arucoDetectedIDs[i] == 8 ) ) {
					...
					if ( arucoDetectedIDs[i] == shared->targetActiveID ) {
						...
						shared->targetScreenPosition  = cv::Point2i( avgX, avgY );
						...
						shared->targetRotationVector	= arucoRotationVector[arucoDetectedIDs[i]];
						shared->targetTranslationVector = arucoTranslationVector[arucoDetectedIDs[i]];
...
// 		// Reset individual tag state
// 		for ( size_t t = 0; t < shared->targetPresent.size(); t++ ) {
// 			shared->targetPresent[t] = false;
// 		}
...
// 			// Draw tags on frame
// 			// cv::aruco::drawDetectedMarkers( shared->matFrameUndistorted, arucoCorners, arucoDetectedIDs );
...
// 				// Only process markers in valid range
// 				if ( ( arucoDetectedIDs[i] > 0 && arucoDetectedIDs[i] <= 8 ) ) {

// 					// Update bool table of targets
// 					shared->targetPresent[arucoDetectedIDs[i]] = true;

// 					// std::cout << "Detected: " << arucoDetectedIDs[i] << "  ID: " << shared->targetActiveID << "\n";

// 					// Only process active ID
// 					if ( arucoDetectedIDs[i] == shared->targetActiveID ) {


// 						// std::cout << "ID: " << ID << " area: " << cv::contourArea( arucoCorners[i], true );
